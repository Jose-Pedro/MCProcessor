...
sap.ui.define([
    ...
    function (Base, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.techCostAnalys.techCostAnalys", {
            onInit: function () {
                let oEventBus = Core.getEventBus()
                oEventBus.subscribe('AGORA_REQUEST', 'REFRESH_PHASE_3', this._onRequestBindingChange, this)
                // oEventBus.subscribe('AGORA_REQUEST', 'PHASE_CLOSED', this._onPhaseClosed, this)
            },

            onExit: function( ) {
                let oEventBus = Core.getEventBus();
                oEventBus.unsubscribe('AGORA_REQUEST', 'REFRESH_PHASE_3', this._onRequestBindingChange, this);
            ...
            _onRequestBindingChange: function (sChannel, sPath, oData) {
                ...
                let elementPath = this._getContextElement('Phases','techCostAnalys', oData.oContext)
            ...
            _onPhaseBindingChange : function (oEvent) {
                ...
                if (oContext) {
                    ...
                    if(iStatus === 3) {
                        oConfigModel.setProperty('/Phases/techCostAnalys/status', 'sap-icon://status-completed')
                    } else {
                        oConfigModel.setProperty('/Phases/techCostAnalys/status', '')
                    }
                    if(iStatus === 2) {
                        oConfigModel.setProperty('/Phases/techCostAnalys/enabled', false)
                    } else {
                        oConfigModel.setProperty('/Phases/techCostAnalys/enabled', true)
                    }
