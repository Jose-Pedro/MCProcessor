...
sap.ui.define([
    ...
    function (Base, UploadSetItem, Messaging, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.techCostAnalys.blocks.dashboard", {
            /*
            ...
            */
            ...
            onCloseBlock: function (oEvent) {
                ...
                let oForm = oView.byId('dashboardForm')
                let aErrors = []
                aErrors = oForm.check()
                if(aErrors.length < 1) {
            ...
            _onPhaseBindingChange : function (sChannel, sPath, oData) {
                ...
                if (oView.getModel('configuration').getData().stepper.firstActivePhase === 'techCostAnalys') {
                    let elementPath = this._getContextElement('Blocks','dashboard', oData.oContext)
                    if (elementPath) {
                        let oOptions = {
                        ...
                        oModel.setProperty('/processInfo/techCostAnalys/dashboard/VISIBLE_ON', '')
                    }
                }
            },

            _bindUploadTable: function (oEvent) {
                let oUploadTable = this.getView().byId('dashboardWorksUploadTable')
                if (oUploadTable) this._bindUploadTableFromView(oEvent.getParameter('data')?.ID, true, 'dashboardWorksUploadTable', 30, 'techCostAnalys', 'dashboard')
            },

            _bindDocumentsPerBlock: function(oEvent) {
                this._bindDocumentsPerBlockFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.dpbVisibleVF, 'dashboardApprovalFlow',  30, 'techCostAnalys', 'dashboard')
            },

            _bindWorksPerBlock: function(oEvent) {
                this._bindWorksFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.worksVisibleVF, 'dashboardWorks',  30, 'techCostAnalys', 'dashboard')
            },

            _bindChecklist: function(oEvent) {
                this._bindChecklistFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.checklistVisibleVF, 'dashboardChecklist',  30, 'techCostAnalys', 'dashboard')
            },

            _onResponsibleChange: function(sChannel, sPath, oData) {
                if(oData.blockProcessFlowId === 'dashboard') {
            ...
            _onRequestDocumentChange: function (sChannel, sPath, oData) {
                ...
                if (oContext) {
                    ...
                    if(sId === oData.blockId && bVisible ) this._bindDocumentsPerBlockFromView(sId, bVisible, 'dashboardApprovalFlow',  30, 'techCostAnalys', 'dashboard')
                }
            },
            ...
            _onActivateSwitch: function (bActivate, oView){
                ...
                if(!bActivate) {
                    ...
                    //NOSONAR let oComponent = oView.byId("dashboardWorks")
