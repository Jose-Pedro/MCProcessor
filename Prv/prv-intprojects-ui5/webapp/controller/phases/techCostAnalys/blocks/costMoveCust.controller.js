...
sap.ui.define([
    ...
    function (Base, UploadSetItem, Messaging, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.techCostAnalys.blocks.costMoveCust", {
            /*
            ...
            */
            ...
            onCloseBlock: function (oEvent) {
                ...
                let oForm = oView.byId('costMoveCustForm')
                let aErrors = []
                aErrors = oForm.check()
                if(aErrors.length < 1) {
            ...
            _onPhaseBindingChange : function (sChannel, sPath, oData) {
                ...
                if (oView.getModel('configuration').getData().stepper.firstActivePhase === 'techCostAnalys') {
                    let elementPath = this._getContextElement('Blocks','costMoveCust', oData.oContext)
                    if (elementPath) {
                        let oOptions = {
                        ...
                        oModel.setProperty('/processInfo/techCostAnalys/costMoveCust/VISIBLE_ON', '')
                    }
                }
            },

            _bindUploadTable: function (oEvent) {
                let oUploadTable = this.getView().byId('costMoveCustUploadTable')
                if (oUploadTable) this._bindUploadTableFromView(oEvent.getParameter('data')?.ID, true, 'costMoveCustUploadTable', 30, 'techCostAnalys', 'costMoveCust')
            },

            _bindDocumentsPerBlock: function(oEvent) {
                this._bindDocumentsPerBlockFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.dpbVisibleVF, 'costMoveCustApprovalFlow', 30, 'techCostAnalys', 'costMoveCust')
            },

            _bindWorksPerBlock: function(oEvent) {
                this._bindWorksFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.worksVisibleVF, 'costMoveCustWorks', 30, 'techCostAnalys', 'costMoveCust')
            },

            _bindChecklist: function(oEvent) {
                this._bindChecklistFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.checklistVisibleVF, 'costMoveCustChecklist', 30, 'techCostAnalys', 'costMoveCust')
            },

            _onResponsibleChange: function(sChannel, sPath, oData) {
                if(oData.blockProcessFlowId === 'costMoveCust') {
            ...
            _onRequestDocumentChange: function (sChannel, sPath, oData) {
                ...
                if (oContext) {
                    ...
                    if(sId === oData.blockId && bVisible ) this._bindDocumentsPerBlockFromView(sId, bVisible, 'costMoveCustApprovalFlow', 30, 'techCostAnalys', 'costMoveCust')
                }
            },
            ...
            _onActivateSwitch: function (bActivate, oView){
                ...
                if(!bActivate) {
                    ...
                    //NOSONAR let oComponent = oView.byId("costMoveCustWorks")
