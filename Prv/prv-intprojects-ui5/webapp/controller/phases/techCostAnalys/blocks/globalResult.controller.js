...
sap.ui.define([
    ...
    function (Base, UploadSetItem, Messaging, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.techCostAnalys.blocks.globalResult", {
            /*
            ...
            */
            ...
            _onPhaseBindingChange : function (sChannel, sPath, oData) {
                ...
                if (oView.getModel('configuration').getData().stepper.firstActivePhase === 'techCostAnalys') {
                    ...
                    if (elementPath) {
                        ...
                        oModel.setProperty('/processInfo/techCostAnalys/globalResult/VISIBLE_ON', '')
                    }
                }
            },

            _bindUploadTable: function (oEvent) {
                let oUploadTable = this.getView().byId('techCostAnalysGlobalResultUploadTable')
                if (oUploadTable) this._bindUploadTableFromView(oEvent.getParameter('data')?.ID, true, 'techCostAnalysGlobalResultUploadTable', 30, 'techCostAnalys', 'globalResult')
            },

            _bindDocumentsPerBlock: function(oEvent) {
                this._bindDocumentsPerBlockFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.dpbVisibleVF, 'techCostAnalysGlobalResultApprovalFlow', 30, 'techCostAnalys', 'globalResult')
            },

            _bindWorksPerBlock: function(oEvent) {
                this._bindWorksFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.worksVisibleVF, 'techCostAnalysGlobalResultWorks', 30, 'techCostAnalys', 'globalResult')
            },

            _bindChecklist: function(oEvent) {
                this._bindChecklistFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.checklistVisibleVF, 'techCostAnalysGlobalResultChecklist', 30, 'techCostAnalys', 'globalResult')
            },
            ...
            _onRequestDocumentChange: function (sChannel, sPath, oData) {
                ...
                if (oContext) {
                    ...
                    if(sId === oData.blockId && bVisible ) this._bindDocumentsPerBlockFromView(sId, bVisible, 'techCostAnalysGlobalResultApprovalFlow', 30, 'techCostAnalys', 'globalResult')
                }
            },
