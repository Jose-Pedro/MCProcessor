...
sap.ui.define([
    ...
    function (Base, UploadSetItem, Messaging, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.techCostAnalys.blocks.costInfraWorks", {
            /*
            ...
            */
            ...
            onCloseBlock: function (oEvent) {
                ...
                let oForm = oView.byId('costInfraWorksForm')
                let aErrors = []
                aErrors = oForm.check()
                if(aErrors.length < 1) {
            ...
            _onPhaseBindingChange : function (sChannel, sPath, oData) {
                ...
                if (oView.getModel('configuration').getData().stepper.firstActivePhase === 'techCostAnalys') {
                    let elementPath = this._getContextElement('Blocks','costInfraWorks', oData.oContext)
                    if (elementPath) {
                        let oOptions = {
                        ...
                        oModel.setProperty('/processInfo/techCostAnalys/costInfraWorks/VISIBLE_ON', '')
                    }
                }
            },

            _bindUploadTable: function (oEvent) {
                let oUploadTable = this.getView().byId('costInfraWorksUploadTable')
                if (oUploadTable) this._bindUploadTableFromView(oEvent.getParameter('data')?.ID, true, 'costInfraWorksUploadTable', 30, 'techCostAnalys', 'costInfraWorks')
            },

            _bindDocumentsPerBlock: function(oEvent) {
                this._bindDocumentsPerBlockFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.dpbVisibleVF, 'costInfraWorksApprovalFlow', 30, 'techCostAnalys', 'costInfraWorks')
            },

            _bindWorksPerBlock: function(oEvent) {
                this._bindWorksFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.worksVisibleVF, 'costInfraWorksWorks', 30, 'techCostAnalys', 'costInfraWorks')
            },

            _bindChecklist: function(oEvent) {
                this._bindChecklistFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.checklistVisibleVF, 'costInfraWorksChecklist', 30, 'techCostAnalys', 'costInfraWorks')
            },

            _onResponsibleChange: function(sChannel, sPath, oData) {
                if(oData.blockProcessFlowId === 'costInfraWorks') {
            ...
            _onRequestDocumentChange: function (sChannel, sPath, oData) {
                ...
                if (oContext) {
                    ...
                    if(sId === oData.blockId && bVisible ) this._bindDocumentsPerBlockFromView(sId, bVisible, 'costInfraWorksApprovalFlow', 30, 'techCostAnalys', 'costInfraWorks')
                }
            },
            ...
            _onActivateSwitch: function (bActivate, oView){
                ...
                if(!bActivate) {
                    ...
                    //NOSONAR let oComponent = oView.byId("costInfraWorksWorks")
