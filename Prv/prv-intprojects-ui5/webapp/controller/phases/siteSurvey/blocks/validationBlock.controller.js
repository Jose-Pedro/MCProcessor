...
sap.ui.define([
    ...
    function (Base, UploadSetItem, Messaging, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.siteSurvey.blocks.validationBlock", {
            /*
            ...
            */
            ...
            _onPhaseBindingChange : function (sChannel, sPath, oData) {
                ...
                if (oView.getModel('configuration').getData().stepper.firstActivePhase === 'siteSurvey') {
                    let elementPath = this._getContextElement('Blocks','validationBlock', oData.oContext)
                    if (elementPath) {
                        let oOptions = {
                        ...
                        oModel.setProperty('/processInfo/siteSurvey/validationBlock/VISIBLE_ON', '')
                    }
                }
            },

            _bindUploadTable: function (oEvent) {
                let oUploadTable = this.getView().byId('validationBlockUploadTable')
                if (oUploadTable) this._bindUploadTableFromView(oEvent.getParameter('data')?.ID, true, 'validationBlockUploadTable', 30, 'siteSurvey', 'validationBlock')
            },

            _bindDocumentsPerBlock: function(oEvent) {
                this._bindDocumentsPerBlockFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.dpbVisibleVF, 'validationBlockApprovalFlow', 30, 'siteSurvey', 'validationBlock')
            },

            _bindWorksPerBlock: function(oEvent) {
                this._bindWorksFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.worksVisibleVF, 'validationBlockWorks', 30, 'siteSurvey', 'validationBlock')
            },

            _bindChecklist: function(oEvent) {
                this._bindChecklistFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.checklistVisibleVF, 'validationBlockChecklist', 30, 'siteSurvey', 'validationBlock')
            },

            _onResponsibleChange: function(sChannel, sPath, oData) {
                if(oData.blockProcessFlowId === 'validationBlock') {
            ...
            _onRequestDocumentChange: function (sChannel, sPath, oData) {
                ...
                if (oContext) {
                    ...
                    if(sId === oData.blockId && bVisible ) this._bindDocumentsPerBlockFromView(sId, bVisible, 'validationBlockApprovalFlow', 30, 'siteSurvey', 'validationBlock')
                }
            },

            _onActivateSwitch: function (bActivate, oView){
                let oForm = oView.byId('validationBlockForm')
                if(!bActivate) {
                    if(oForm) oForm.setEditable(false)
                } else {
                    if(oForm) oForm.setEditable(true)
                    //NOSONAR let oComponent = oView.byId("validationBlockWorks")
