...
sap.ui.define([
    ...
    function (Base, UploadSetItem, Messaging, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.feasibilCheck.blocks.permits", {
            /*
            ...
            */
            ...
            onCloseBlock: function (oEvent) {
                ...
                let oForm = oView.byId('permitsForm')
                let aErrors = []
                aErrors = oForm.check()
                if(aErrors.length < 1) {
            ...
            _onPhaseBindingChange : function (sChannel, sPath, oData) {
                ...
                if (oView.getModel('configuration').getData().stepper.firstActivePhase === 'feasibilCheck') {
                    let elementPath = this._getContextElement('Blocks','permits', oData.oContext)
                    if (elementPath) {
                        let oOptions = {
                            path: oData.oContext.getDeepPath() + '/' + elementPath,
                            parameters: { groupId: 'blockRead', expand: 'BlockProvision,BlockStatus,BlockProvision/ApproverTypes,BlockProvision/SubcoTypes,BlockProvision/PermitsFeasibilities,BlockProvision/PermitsNeeded,BlockProvision/PermitsFeasibilityExplanations' },
                        ...
                        oModel.setProperty('/processInfo/feasibilCheck/permits/VISIBLE_ON', '')
                    }
                }
            },

            _bindUploadTable: function (oEvent) {
                let oUploadTable = this.getView().byId('permitsUploadTable')
                if (oUploadTable) this._bindUploadTableFromView(oEvent.getParameter('data')?.ID, true, 'permitsUploadTable', 30, 'feasibilCheck', 'permits')
            },

            _bindDocumentsPerBlock: function(oEvent) {
                this._bindDocumentsPerBlockFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.dpbVisibleVF, 'permitsApprovalFlow', 30, 'feasibilCheck', 'permits')
            },

            _bindWorksPerBlock: function(oEvent) {
                this._bindWorksFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.worksVisibleVF, 'permitsWorks', 30, 'feasibilCheck', 'permits')
            },

            _bindChecklist: function(oEvent) {
                this._bindChecklistFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.checklistVisibleVF, 'permitsChecklist', 30, 'feasibilCheck', 'permits')
            },

            _onResponsibleChange: function(sChannel, sPath, oData) {
                if(oData.blockProcessFlowId === 'permits') {
            ...
            _onRequestDocumentChange: function (sChannel, sPath, oData) {
                ...
                if (oContext) {
                    ...
                    if(sId === oData.blockId && bVisible ) this._bindDocumentsPerBlockFromView(sId, bVisible, 'permitsApprovalFlow', 30, 'feasibilCheck', 'permits')
                }
            },
            ...
            _onActivateSwitch: function (bActivate, oView){
                ...
                if(!bActivate) {
                    ...
                    //NOSONAR let oComponent = oView.byId("permitsWorks")
