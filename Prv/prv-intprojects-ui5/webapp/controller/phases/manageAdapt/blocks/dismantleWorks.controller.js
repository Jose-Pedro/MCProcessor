...
sap.ui.define([
    ...
    function (Base, UploadSetItem, Messaging, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.manageAdapt.blocks.dismantleWorks", {
            /*
            ...
            */
            ...
            onCloseBlock: function (oEvent) {
                ...
                let oForm = oView.byId('dismantleWorksForm')
                let aErrors = []
                aErrors = oForm.check()
                if(aErrors.length < 1) {
            ...
            _onPhaseBindingChange : function (sChannel, sPath, oData) {
                ...
                if (oView.getModel('configuration').getData().stepper.firstActivePhase === 'manageAdapt') {
                    let elementPath = this._getContextElement('Blocks','dismantleWorks', oData.oContext)
                    if (elementPath) {
                        let oOptions = {
                        ...
                        oModel.setProperty('/processInfo/manageAdapt/dismantleWorks/VISIBLE_ON', '')
                    }
                }
            },

            _bindUploadTable: function (oEvent) {
                let oUploadTable = this.getView().byId('dismantleWorksUploadTable')
                if (oUploadTable) this._bindUploadTableFromView(oEvent.getParameter('data')?.ID, true, 'dismantleWorksUploadTable', 30, 'manageAdapt', 'dismantleWorks')
            },

            _bindDocumentsPerBlock: function(oEvent) {
                this._bindDocumentsPerBlockFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.dpbVisibleVF, 'dismantleWorksApprovalFlow', 30, 'manageAdapt', 'dismantleWorks')
            },

            _bindWorksPerBlock: function(oEvent) {
                this._bindWorksFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.worksVisibleVF, 'dismantleWorksWorks', 30, 'manageAdapt', 'dismantleWorks')
            },

            _bindChecklist: function(oEvent) {
                this._bindChecklistFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.checklistVisibleVF, 'dismantleWorkChecklist', 30, 'manageAdapt', 'dismantleWorks')
            },

            _onResponsibleChange: function(sChannel, sPath, oData) {
                if(oData.blockProcessFlowId === 'dismantleWorks') {
            ...
            _onRequestDocumentChange: function (sChannel, sPath, oData) {
                ...
                if (oContext) {
                    ...
                    if(sId === oData.blockId && bVisible ) this._bindDocumentsPerBlockFromView(sId, bVisible, 'dismantleWorksApprovalFlow', 30, 'manageAdapt', 'dismantleWorks')
                }
            },
            ...
            _onActivateSwitch: function (bActivate, oView){
                ...
                if(!bActivate) {
                    ...
                    //NOSONAR let oComponent = oView.byId("dismantleWorksWorks")
