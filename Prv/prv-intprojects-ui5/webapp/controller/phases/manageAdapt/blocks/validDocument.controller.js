...
sap.ui.define([
    ...
    function (Base, UploadSetItem, Messaging, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.manageAdapt.blocks.validDocument", {
            /*
            ...
            */
                ...
                if (oView.getModel('configuration').getData().stepper.firstActivePhase === 'manageAdapt') {
                    ...
                    if (elementPath) {
                        let oOptions = {
                            ...
                            parameters: { groupId: 'blockRead', expand: 'BlockProvision,BlockStatus,BlockProvision/ApproverTypes,BlockProvision/SubcoTypes,BlockProvision/MadResults' },
                        ...
                        oModel.setProperty('/processInfo/manageAdapt/validDocument/VISIBLE_ON', '')
                    }
                }
            },

            _bindUploadTable: function (oEvent) {
                let oUploadTable = this.getView().byId('manageAdaptValidDocumentUploadTable')
                if (oUploadTable) this._bindUploadTableFromView(oEvent.getParameter('data')?.ID, true, 'manageAdaptValidDocumentUploadTable', 30, 'manageAdapt', 'validDocument')
            },

            _bindDocumentsPerBlock: function(oEvent) {
                this._bindDocumentsPerBlockFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.dpbVisibleVF, 'validDocumentApprovalFlow', 30, 'manageAdapt', 'validDocument')
            },

            _bindWorksPerBlock: function(oEvent) {
                this._bindWorksFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.worksVisibleVF, 'validDocumentWorks', 30, 'manageAdapt', 'validDocument')
            },

            _bindChecklist: function(oEvent) {
                this._bindChecklistFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.checklistVisibleVF, 'validDocumentChecklist', 30, 'manageAdapt', 'validDocument')
            },
            ...
            _onRequestDocumentChange: function (sChannel, sPath, oData) {
                ...
                if (oContext) {
                    ...
                    if(sId === oData.blockId && bVisible ) this._bindDocumentsPerBlockFromView(sId, bVisible, 'validDocumentApprovalFlow', 30, 'manageAdapt', 'validDocument')
                }
            },
