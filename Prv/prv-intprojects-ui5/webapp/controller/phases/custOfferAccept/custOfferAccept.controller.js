...
sap.ui.define([
    ...
    function (Base, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.custOfferAccept.custOfferAccept", {
            onInit: function () {
                let oEventBus = Core.getEventBus()
                oEventBus.subscribe('AGORA_REQUEST', 'REFRESH_PHASE_4', this._onRequestBindingChange, this)
                // oEventBus.subscribe('AGORA_REQUEST', 'PHASE_CLOSED', this._onPhaseClosed, this)
            },

            onExit: function( ) {
                let oEventBus = Core.getEventBus();
                oEventBus.unsubscribe('AGORA_REQUEST', 'REFRESH_PHASE_4', this._onRequestBindingChange, this);
            ...
            _onRequestBindingChange: function (sChannel, sPath, oData) {
                ...
                //NOSONAR     }
                //NOSONAR }
                let elementPath = this._getContextElement('Phases','custOfferAccept', oData.oContext)
            ...
            _onPhaseBindingChange : function (oEvent) {
                ...
                if (oContext) {
                    ...
                    if(iStatus === 3) {
                        oConfigModel.setProperty('/Phases/custOfferAccept/status', 'sap-icon://status-completed')
                    } else {
                        oConfigModel.setProperty('/Phases/custOfferAccept/status', '')
                    }
                    if(iStatus === 2) {
                        oConfigModel.setProperty('/Phases/custOfferAccept/enabled', false)
                    } else {
                        oConfigModel.setProperty('/Phases/custOfferAccept/enabled', true)
                    }
