...
sap.ui.define([
    ...
    function (Base, UploadSetItem, Messaging, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.finalValidation.blocks.validDocument", {
            /*
                onInit --> Subscribe to REQUEST_CHANGE_EVENT
            */
            onInit: function () {
                let oEventBus = Core.getEventBus()
                oEventBus.subscribe('AGORA_REQUEST', 'PHASE_7_BINDING_CHANGE', this._onPhaseBindingChange, this)
            ...
            onExit: function () {
                let oEventBus = Core.getEventBus()
                oEventBus.unsubscribe('AGORA_REQUEST', 'PHASE_7_BINDING_CHANGE', this._onPhaseBindingChange, this)
            ...
            _onPhaseBindingChange : function (sChannel, sPath, oData) {
                ...
                if (oView.getModel('configuration').getData().stepper.firstActivePhase === 'finalValidation') {
                    let elementPath = this._getContextElement('Blocks','validDocument', oData.oContext)
                    if (elementPath) {
                        let oOptions = {
                        ...
                        oModel.setProperty('/processInfo/finalValidation/validDocument/VISIBLE_ON', '')
                    }
                }
            },

            _bindUploadTable: function (oEvent) {
                let oUploadTable = this.getView().byId('finalValidationValidDocumentUploadTable')
                if (oUploadTable) this._bindUploadTableFromView(oEvent.getParameter('data')?.ID, true, 'finalValidationValidDocumentUploadTable', 30, 'finalValidation', 'validDocument')
            },

            _bindDocumentsPerBlock: function(oEvent) {
                this._bindDocumentsPerBlockFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.dpbVisibleVF, 'finalValidationValidDocumentApprovalFlow', 30, 'finalValidation', 'validDocument')
            },

            _bindWorksPerBlock: function(oEvent) {
                this._bindWorksFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.worksVisibleVF, 'finalValidationValidDocumentWorks', 30, 'finalValidation', 'validDocument')
            },

            _bindChecklist: function(oEvent) {
                this._bindChecklistFromView(oEvent.getParameter('data')?.ID, oEvent.getParameter('data')?.checklistVisibleVF, 'finalValidationValidDocumentChecklist', 30, 'finalValidation', 'validDocument')
            },

            _onResponsibleChange: function(sChannel, sPath, oData) {
                if(oData.blockProcessFlowId === 'validDocument') {
            ...
            _onRequestDocumentChange: function (sChannel, sPath, oData) {
                ...
                if (oContext) {
                    ...
                    if(sId === oData.blockId && bVisible ) this._bindDocumentsPerBlockFromView(sId, bVisible, 'finalValidationValidDocumentApprovalFlow', 30, 'finalValidation', 'validDocument')
                }
            },

            _onActivateSwitch: function (bActivate, oView){
                let oForm = oView.byId('validDocumentForm')
                if(!bActivate) {
                    if(oForm) oForm.setEditable(false)
                } else {
                    if(oForm) oForm.setEditable(true)
                    //NOSONAR let oComponent = oView.byId("validDocumentWorks")
