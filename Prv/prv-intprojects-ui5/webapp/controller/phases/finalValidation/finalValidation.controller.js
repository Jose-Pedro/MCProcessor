...
sap.ui.define([
    ...
    function (Base, Core) {
        ...
        return Base.extend("prvintprojectsui5.controller.phases.finalValidation.finalValidation", {
            onInit: function () {
                let oEventBus = Core.getEventBus()
                oEventBus.subscribe('AGORA_REQUEST', 'REFRESH_PHASE_7', this._onRequestBindingChange, this)
                // oEventBus.subscribe('AGORA_REQUEST', 'PHASE_CLOSED', this._onPhaseClosed, this)
            },

            onExit: function( ) {
                let oEventBus = Core.getEventBus();
                oEventBus.unsubscribe('AGORA_REQUEST', 'REFRESH_PHASE_7', this._onRequestBindingChange, this)
            ...
            _onRequestBindingChange: function (sChannel, sPath, oData) {
                ...
                let elementPath = this._getContextElement('Phases','finalValidation', oData.oContext)
            ...
            _onPhaseBindingChange : function (oEvent) {
                ...
                if (oContext) {
                    ...
                    if(iStatus === 3) {
                        oConfigModel.setProperty('/Phases/finalValidation/status', 'sap-icon://status-completed')
                    } else {
                        oConfigModel.setProperty('/Phases/finalValidation/status', '')
                    }
                    if(iStatus === 2) {
                        oConfigModel.setProperty('/Phases/finalValidation/enabled', false)
                    } else {
                        oConfigModel.setProperty('/Phases/finalValidation/enabled', true)
                    }
