...
if (-not $OutFile) { $OutFile = [System.IO.Path]::ChangeExtension($src.Path, '.docx') }
...
$sections = @(
    ...
    'Perfil','Línguas','Linguas','Experiência','Experiencia','Educação','Educacao',
    'Conquistas','Realizações','Realizacoes','Competências','Competencias',
    'Referências a pedido','Referencias a pedido','Sumário','Sumario','Certificações','Certificacoes'
)
...
$tmpHtml = Join-Path $env:TEMP ("md2docx_" + [guid]::NewGuid().ToString('N').Substring(0,8) + ".html")
...
try {
    ...
    # wdFormatXMLDocument = 12 (.docx without macros)
    $doc.SaveAs2($OutFile, 12)
    $doc.Close($false)
    $doc = $null
} finally {
    if ($doc)  { try { $doc.Close($false) } catch {} ; [void][System.Runtime.InteropServices.Marshal]::ReleaseComObject($doc) }
...
if (Test-Path -LiteralPath $OutFile) {
    ...
    Write-Warning "DOCX not produced; tmp html kept for inspection: $tmpHtml"
    throw "DOCX was not produced: $OutFile"
}
