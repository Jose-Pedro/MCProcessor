# build-mcounterpart-doc-v2.ps1 - regenerate MCounterPart-Architecture-v2.docx
# at workspace root. V2 differences vs v1:
#   - Glossary section up front (explicit nickname definitions)
#   - CModel ("Context Model") and CHosts/CloudModels split clearly
#   - Subagent fan-out shown in the diagram
#   - "CHost" (physical Desktop/server, singular) disambiguated from
#     "CHosts / CloudModels" (cloud hosts, remote scalable pool, plural)
...
if (-not $OutPath) { $OutPath = Join-Path $root 'MCounterPart-Architecture-v2.docx' }
$tmp       = $env:TEMP
$mmd       = Join-Path $tmp 'mcounterpart-diag-v2.mmd'
$png       = Join-Path $tmp 'mcounterpart-diag-v2.png'
$svgOut    = Join-Path $root 'MCounterPart-Architecture-Diagram-v2.svg'
...
flowchart TB
    ...
    subgraph P1 [Pair 1 - Architect + Agent]
      ...
      A1[CHost Agent<br/>VS Code on Desktop]:::agent
      H1 <--> A1
    end
    subgraph P2 [Pair 2 - Architect + Agent]
      H2[Juan<br/>architect]:::human
      A2[Laptop Agent<br/>VS Code on Laptop]:::agent
      H2 <--> A2
    end
    ...
    subgraph DESK [CHost - physical Desktop / server, always-on]
      direction TB
    ...
    subgraph CHOSTS [CHosts / CloudModels - cloud hosts, remote scalable pool]
      direction LR
    ...
    subgraph SUBAG [Subagents - spawned by a CHost slot to parallelize work]
    ...
    subgraph LADDER [Routing ladder R3 - cheapest tier first]
      L1[1. CModel local] --> L2[2. Counterpart agent] --> L3[3. Scheduled task] --> L4[4. Clone-queue] --> L5[5. Inline CHost slot]
    end
'@
...
# 3. Build .docx via Word COM through the v2 content helper
$helper = Join-Path $scriptDir 'build-mcounterpart-doc-v2.content.ps1'
if (-not (Test-Path $helper)) { throw "content helper missing: $helper" }
. $helper
Write-MCounterPartDocV2 -OutPath $OutPath -DiagramPng $png
