# build-mcounterpart-doc-v2.content.ps1 - dot-sourced by build-mcounterpart-doc-v2.ps1.
# Exposes Write-MCounterPartDocV2 which uses Word COM to render the v2 spec.
function Write-MCounterPartDocV2 {
    ...
    try {
        ...
        P ("Architecture v2 - {0}" -f (Get-Date -Format yyyy-MM-dd))
        P "v2 changes vs v1: explicit glossary of nicknames; CModel (local Context Model) and CHosts/CloudModels (remote cloud-host pool) split clearly; subagent fan-out shown; CHost (the physical Desktop/server) disambiguated from CHosts (cloud hosts) despite the similar spelling."
        $s.InsertNewPage()

        H1 "1. Executive summary"
        P "MCounterPart bonds each human architect to one dedicated Augment Agent. Pairs share one workspace, one memory, one rule set; coordination is through file-based inboxes and scheduled background work. The product scales by adding pairs, not by scaling a single agent. Cost is minimized via a 5-tier routing ladder: CModel (local) -> counterpart agent -> scheduled task -> clone-queue -> inline CHosts slot. The standing rule is: always pick the cheapest-token path, and continuously train CHost-local helpers (CModel, scheduled scripts, future MCP-Worker daemon) so they handle more work over time with less agent involvement."

        H1 "2. Glossary - naming conventions"
        P "Several short names are reused in the codebase and in chat. The disambiguation that matters most: CHost (singular, capital H) is the physical machine; CHosts (plural, also spelled CloudModels) are remote cloud-hosted models. The names look alike on purpose - the agent's local home (CHost) and its remote work-pool (CHosts) are siblings on the same compute spectrum."
        Tbl @(
            @("Term","Meaning"),
            ...
            @("Pair","1 human architect + 1 dedicated Augment Agent on 1 host. Today: Jose+CHost-Agent and Juan+Laptop-Agent."),
            @("Architect","A human user of the workspace (Jose, Juan)."),
            @("Counterpart agent","The OTHER pair's agent. For the CHost-Agent, the counterpart is the Laptop-Agent (different host, different human, different subscription) and vice versa."),
            @("CHost (singular)","The always-on physical Desktop / server. Hosts CModel and all scheduled tasks. Used in env var AUGMENT_AGENT_HOST=chost on this machine."),
            @("CModel","Context Model. The local model pair (writer + retriever) kept loaded in CHost memory by Ollama. Config, prompts, and state synced via git + OneDrive (SharePoint). Free, slow, the first tier of the routing ladder."),
            @("CHosts / CloudModels","Cloud Hosts. The remote scalable model pool (airouter today). Multiple keys, multiple slots per key; can be incremented at will by adding keys or upgrading models. Acts as the agent's primary heavy-lift counterpart for work above the local ceiling."),
            @("Slot","One concurrent request capacity on a CHosts key. Airouter today: 3 slots per key."),
            @("K1 / K2","The two airouter API keys (today). K1 = 3 slots, K2 = 3 slots. Total = 6 concurrent slots."),
            @("Subagent","A worker spawned BY a CHost slot (typically via the clone-agent queue) to parallelize work for the main agent. Subagents are short-lived, single-purpose, and report results back through the inbox or queue files."),
            ...
            @("Routing ladder R3","The 5-tier escalation rule in agent-operating-rules.md: 1.CModel -> 2.counterpart agent -> 3.scheduled task -> 4.clone-queue -> 5.inline CHost slot."),
        ...
        $s.TypeParagraph()
        P "Notation: rounded blocks = agents; oval-tagged tall blocks = humans; cylinders = shared persistent state; rose blocks = CModel (local on CHost); purple blocks = CHosts / CloudModels (remote cloud-host slots); sand blocks = subagents spawned by a CHost slot; tan blocks = scheduled tasks; dashed arrows = optional / on-demand; solid arrows = always-on dependencies."
        ...
        H1 "6. CModel - Context Model (local on CHost)"
        P "Definition. CModel is the 'Context Model' - a local model pair kept loaded in CHost memory by Ollama so it is always ready, with no cold-start cost. Its full configuration, prompt templates, and operational state live in the workspace and are synced through git and OneDrive/SharePoint, so any agent reading the workspace can reproduce and call it identically."
        P "Composition. CModel-writer = qwen2.5-coder:7b-instruct-q4_K_M (generation). CModel-retriever = nomic-embed-text (embeddings). Served by Ollama at 127.0.0.1:11434 on CHost; the laptop reaches the same endpoint via Tailscale at 100.83.6.49:11434."
        ...
        P "Role in the ladder. CModel is tier 1. Anything the agent can plausibly delegate here goes here, even when it is slower than a CHosts slot. The fleet improves over time (better prompts, sharper decomposition, eventual fine-tunes) so the remote-call rate trends downward, not up."

        H1 "7. CHosts / CloudModels - cloud hosts (remote scalable pool)"
        P "Definition. CHosts (a.k.a. CloudModels) are the remote scalable models the agent escalates to. Today this is the airouter pool: 2 API keys, 3 concurrent slots per key, 6 slots total. The pool can be incremented at any time by adding keys, opening more slots, or swapping in new models per purpose. CHosts act as the agent's primary heavy-lift counterpart - the place real reasoning, large refactors, and multi-file judgment calls go when CModel cannot do them."
        P "Subagent capability. A CHost slot can act as an orchestrator that spawns subagents - short-lived single-purpose workers (today drained from the clone-agent queue) that fan out a unit of work in parallel and report results back through the shared workspace. Subagents are owned by the slot that spawned them and never persist context across runs."
        Tbl @(
            ...
            @("3","K1","Clone-agent worker drain (subagent host)","CHost background queue, every 10 min","Qwen3.6"),
        ...
        H1 "8. Subagents"
        P "A subagent is a worker spawned by a CHost slot (or by a scheduled task draining the clone-queue into a slot) to parallelize a single unit of work for the main agent. Subagents are short-lived, single-purpose, and report results back through the inbox queues or done.jsonl. They do not share context with the main agent across runs, which is by design - it keeps their cost bounded and their failure modes contained."
        P "Today's subagent host is slot 3 (clone-agent worker drain). The roadmap MCP-Worker daemon will expand the subagent pattern to CHost-local Python / PowerShell workers that pick up file-dropped jobs from a watched folder, costing zero agent tokens on re-runs."
        ...
        P "Routing ladder R3 cheapest tier first: (1) CModel local, (2) Counterpart agent, (3) Scheduled task, (4) Clone-queue, (5) Inline CHost slot. Skip a tier only with a stated reason."
        ...
        P "Done: counterpart-first review-loop, JSONL inboxes, toast notifier, CModel fleet on CHost, scheduled cold-path / clone-worker / auto-checkpoint, OneDrive + git two-tier persistence, session-briefing single-read, v1 architecture doc."
